...
class WorkspaceApi {
    ...
    async joinWorkspaceWithInviteLink(inviteId: string) {
        try {
            const workspace = await api.post(`${this.baseUrl}/${inviteId}/join`);
            ...
            console.log("Error joining workspace: ", err.response.data.message);
            throw new Error(err.response.data.message);
